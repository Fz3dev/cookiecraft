...
describe('GTMConsentMode', () => {
  ...
  test('setDefaultConsent pushes all denied defaults', () => {
    ...
    expect(pushConsentSpy).toHaveBeenCalledWith('default', expect.objectContaining({
      ad_storage: 'denied',
      ad_user_data: 'denied',
  ...
  test('setDefaultConsent pushes url_passthrough when configured', () => {
    const gtm = new GTMConsentMode(dlm, makeConfig({ gtmUrlPassthrough: true }));
    gtm.setDefaultConsent();
  ...
  test('setDefaultConsent pushes ads_data_redaction when configured', () => {
    const gtm = new GTMConsentMode(dlm, makeConfig({ gtmAdsDataRedaction: true }));
    gtm.setDefaultConsent();
  ...
  test('updateConsent maps analytics=true correctly', () => {
    ...
    expect(pushConsentSpy).toHaveBeenCalledWith('update', expect.objectContaining({
  ...
  test('updateConsent maps marketing=true correctly', () => {
    const gtm = new GTMConsentMode(dlm, makeConfig());
    const cats: ConsentCategories = { necessary: true, analytics: false, marketing: true };
    gtm.updateConsent(cats);
  ...
  test('functionality_storage follows preferences when preferences category exists', () => {
    const config = makeConfig({
      categories: {
        ...
        analytics: { enabled: true, readOnly: false, label: 'Analytics', description: '' },
        marketing: { enabled: true, readOnly: false, label: 'Marketing', description: '' },
        preferences: { enabled: true, readOnly: false, label: 'Preferences', description: '' },
      },
    });
    const gtm = new GTMConsentMode(dlm, config);
    const cats: ConsentCategories = { necessary: true, analytics: false, marketing: false, preferences: false };
    gtm.updateConsent(cats);
