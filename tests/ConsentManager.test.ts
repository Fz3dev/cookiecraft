...
describe("ConsentManager", () => {
  ...
  test("validateConsent coerces values to booleans", () => {
    const cats = {
      ...
      analytics: 1,
      marketing: "yes",
    } as unknown as ConsentCategories;
  ...
  test("expiry date defaults to ~6 months in the future", () => {
    const cats: ConsentCategories = {
      necessary: true,
      analytics: false,
      marketing: false,
  ...
  test("expiry date honors consentExpiryMonths config", () => {
    const cm13 = new ConsentManager({ ...baseConfig, consentExpiryMonths: 13 });
    const cats: ConsentCategories = {
      necessary: true,
      analytics: false,
      marketing: false,
    };
    const record = cm13.updateConsent(cats);
    const expiry = new Date(record.expiresAt);
    const now = new Date();
    const diffMonths =
      (expiry.getFullYear() - now.getFullYear()) * 12 +
